...
public class Pedido {
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += ItemDePedidos[i].getPrecoVenda();
		}
	...
	public void mesclarPedido(Pedido outroPedido){
		...
		if (quantProdutos < MAX_PRODUTOS) {
			ItemDePedidos[quantProdutos++] = outroPedido.ItemDePedidos;
	...
	public void imprimirRecibo (Pedido Pedido) {
		...
		double valorPedido = Pedido.valorFinal();


		if (quantProdutos > 10){
			  valorPedido = valorPedido - ((valorPedido/100) * 5);
		...
		for (int i = 0; i < quantProdutos; i++) {
			if (ItemDePedidos[i] != null)
				System.out.println(String.format("%02d - %s", (i + 1), ItemDePedidos[i].toString(),valorPedido));
		}
